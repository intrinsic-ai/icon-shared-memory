...
//   handles that may still be attached to it.
...
class FuturePromiseContext {
 public:
  ...
  tl::expected<RealtimePromise<T>, RealtimeStatus> GetPromise(
    ...
    return RealtimePromise<T>(this, is_reusable);
  }
  ...
  tl::expected<RealtimeFuture<T>, RealtimeStatus> GetFuture(
    ...
    return RealtimeFuture<T>(this, is_reusable);
  }
  ...
  Status Reset(std::chrono::milliseconds timeout) INTRINSIC_NON_REALTIME_ONLY {
    ...
    // Drain any pending has_value notifications.
    (void)has_value_.TryWait();
    (void)future_detached_.TryWait();
    (void)promise_detached_.TryWait();
  ...
  RealtimeStatus TestOnlyResetLock() INTRINSIC_NON_REALTIME_ONLY
    ...
    status = intrinsic::OverwriteIfNotError(status, get_promise_futex_.Lock());
    status = intrinsic::OverwriteIfNotError(status, read_value_futex_.Lock());
    status = intrinsic::OverwriteIfNotError(status, write_value_futex_.Lock());
    status = intrinsic::OverwriteIfNotError(status, cancel_futex_.Lock());
    return status;
  }
  ...
  RealtimeStatus TestOnlyResetUnlock() INTRINSIC_CHECK_REALTIME_SAFE
    ...
    status =
        intrinsic::OverwriteIfNotError(status, write_value_futex_.Unlock());
    status = intrinsic::OverwriteIfNotError(status, read_value_futex_.Unlock());
    status =
        intrinsic::OverwriteIfNotError(status, get_promise_futex_.Unlock());
    status = intrinsic::OverwriteIfNotError(status, get_future_futex_.Unlock());
    return status;
  }
 ...
 private:
  ...
  // this class's methods and the `Reset()` method. The goal is that the
  // `Reset()` method (being the non-realtime method) is the only one requiring
  // all the locks, while the other methods can use the individual locks. All
  // other members of this class are atomic or otherwise thread-safe and do not
 ...
 public:
  ...
  RealtimeStatus Set(T value) INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    *element = std::move(value);
    ...
    INTR_RETURN_STATUS_IF_ERROR(context_->WriteUnlock());
    if (!is_reusable_) {
      status = intrinsic::OverwriteIfNotError(status, Detach());
    }
    return status;
  }
