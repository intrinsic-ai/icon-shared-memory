...
#ifndef PLATFORM_COMMON_BUFFERS_RT_QUEUE_H_
#define PLATFORM_COMMON_BUFFERS_RT_QUEUE_H_
...
// thread safe for a single producer and single consumer (SPSC). It can be used
...
//
// The template type `T` must be copyable. If `T` is not *trivially* copyable,
// then `Insert()` and `Pop()` will not be realtime safe, however it's still
// possible to read from and write to the queue in a realtime safe way; see
// below for details.
//
// Basic Usage:
//
...
// RealtimeQueue<int>::Writer* writer = queue->writer();
//
...
// }
//
// If `PrepareInsert()` returns a non-null value, it must be followed by a call
// to `FinishInsert()` before it can be called again. Likewise, if `Front()`
// returns a non-null value, it must be followed by a call to `DropFront()` or
// `KeepFront()` before it can be called again. If you do not pair up the calls
// correctly (eg, calling `PrepareInsert()` twice without calling
// `FinishInsert()`, calling `DropFront()` without first calling `Front()`,
// etc), it will abort. If `PrepareInsert()` or `Front()` return nullptr, then
// you should *not* call `FinishInsert()`/`DropFront()`/`KeepFront()`, as shown
// above.
//
// You may mix the access methods too - e.g., a non-realtime writing thread can
// use `Insert()` while a realtime reading thread uses `Front()/`DropFront()`.
//
// The `Reader` and `Writer` classes are thread compatible, so you can have
// multiple non-realtime readers or writers by protecting their access with an
// external mutex. It is not possible to have multiple realtime readers or
// writers.
template <typename T>
 ...
 public:
  ...
  class Reader {
   public:
    ...
    // KeepFront, or DropFront, must be called before another call to `Front()`
    // is allowed.
    INTR_MUST_USE_RESULT T* Front() INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    //
    // After calling `MoveFront()`, the object returned by `PrepareInsert()`
    // must not be reused but an object must be assigned to it using the
    // assignment operator.
    ...
    void KeepFront() INTRINSIC_CHECK_REALTIME_SAFE { buffer_.KeepFront(); }
    // Removes the front element; no-op if the queue is empty.
    void DropFront() INTRINSIC_CHECK_REALTIME_SAFE { buffer_.DropFront(); }
    ...
    // objects; use `Front()`/`DropFront()` for realtime safety with non-trivial
    // types.
    INTR_MUST_USE_RESULT std::optional<T> Pop();
    // Returns true when the buffer is empty.
    bool Empty() const INTRINSIC_CHECK_REALTIME_SAFE { return buffer_.Empty(); }
  ...
  class Writer {
   public:
    ...
    INTR_MUST_USE_RESULT T* PrepareInsert() INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    void FinishInsert() INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    // `PrepareInsert()`/`FinishInsert()` for realtime safety with non-trivial
    // types.
  ...
  explicit RealtimeQueue(
      std::optional<size_t> capacity = std::nullopt,
      const std::function<void(T*)>& init_function = nullptr);

  // Get a pointer to the reader.
  Reader* reader() INTRINSIC_CHECK_REALTIME_SAFE { return &reader_; }

  // Get a pointer to the writer.
  Writer* writer() INTRINSIC_CHECK_REALTIME_SAFE { return &writer_; }
  ...
  bool Full() const INTRINSIC_CHECK_REALTIME_SAFE { return buffer_.Full(); }

  // Returns the number of elements in the buffer. Thread-safe.
  size_t Size() const INTRINSIC_CHECK_REALTIME_SAFE { return buffer_.Size(); }

  // Returns the capacity of the buffer.
  size_t Capacity() const INTRINSIC_CHECK_REALTIME_SAFE {
...
#endif  // PLATFORM_COMMON_BUFFERS_RT_QUEUE_H_
