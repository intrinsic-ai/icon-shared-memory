...
// if (active != nullptr) {
//   // Access the contents of the buffer here
// }
// \endcode
//
...
class AsyncBuffer {
 public:
  ...
  // of this call.
  //
  // If nothing has been committed yet, `*buffer` points to a default
  // constructed (potentially uninitialized) `T` afterwards.
  ...
  INTR_MUST_USE_RESULT bool CommitFreeBuffer();
 ...
 private:
  ...
  // Exclusively used by producer thread.
  bool free_buffer_checked_out_ = false;
  // Buffers: active, mailbox, free.
  std::unique_ptr<T> buffers_[3];
};

// Creates a new AsyncBuffer, calling the constructor of `T` with `args` once
// for each element.
//
// (Because this reuses `args` multiple times, we cannot perfect-forward them.)
template <typename T>
template <typename... InitArgs>
...
#ifndef NDEBUG
  INTR_CHECK(current.free_index == next.free_index,
             "free_index of new state is wrong");
  INTR_CHECK(!next.mailbox_full, "mailbox is full when it should be empty");
  INTR_CHECK(next.IsConsistent(), "next state is not consistent");
#endif
  if (buffer == nullptr) {
    return false;
  }
...
#ifndef NDEBUG
  INTR_CHECK(current.active_index == next.active_index,
             "active_index of new state is wrong");
  INTR_CHECK(next.mailbox_full, "mailbox is empty when it should be full");
  INTR_CHECK(next.IsConsistent(), "next state is not consistent");
#endif
