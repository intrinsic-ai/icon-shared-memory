...
// a no-op (e.g. from `kMotionEnabled` to `kMotionEnabling`).
...
enum class HardwareModuleExitCode {
  ...
  // HWM received a restart request.
...
class SharedPromiseWrapper {
 public:
  SharedPromiseWrapper() = default;
  SharedPromiseWrapper(const SharedPromiseWrapper&) = delete;
  SharedPromiseWrapper& operator=(const SharedPromiseWrapper&) = delete;

  // Sets the value of the promise. If the promise is already set, returns an
  // error. This function is thread-safe.
  Status SetValue(T value) {
  ...
  std::shared_future<T> GetSharedFuture() const { return shared_future_; }
 ...
 private:
  ...
  std::optional<std::promise<T>> promise_ INTR_GUARDED_BY(mutex_) =
      std::promise<T>();
  // Must set this after initializing `promise_`, otherwise we'd be
  // dereferencing a nullopt.
